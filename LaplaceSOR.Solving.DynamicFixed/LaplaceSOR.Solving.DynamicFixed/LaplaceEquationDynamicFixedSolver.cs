...
namespace LaplaceSOR.Solving.DynamicFixed;

public class LaplaceEquationDynamicFixedSolver : ILaplaceEquationSolver
{
    private int _height;
    private int _width;
    ...
    private LaplaceEquationDynamicFixedSolver() { }
    public LaplaceEquationDynamicFixedSolver(LaplaceCell[,] initialGrid) => LoadGrid(initialGrid);
    ...
    private double UpdateGrid(double omega, int startY, int endY, bool isWorkingOnRedLaplaceCells)
    {
        ...
        for (int y = startY; y < endY; y++)
        {
            ...
            for (; x < _width - 1; x += 2)
            {
                if (_grid[y, x].Type == CellType.Fixed) continue;
